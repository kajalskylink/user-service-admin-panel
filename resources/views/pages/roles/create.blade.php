<x-admin-layout>
    <div class="content">
        {{-- Breadcrumb --}}
        <x-admin.breadcrumb parent_title="Role List" :parent_route="route('roles.index')" title="Add Role" route="roles.index" />

        <div class="row">
            <div class="col-md-12">
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-white border-bottom py-3">
                        <h4 class="card-title mb-0 fw-bold text-gray-9">Add Role</h4>
                    </div>
                    <form action="{{ route('roles.store') }}" method="POST">
                        ...
                        <div class="card-body">
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control" name="name" value="{{ old('name') }}" required placeholder="Enter role name">
                            </div>
                                                    ...
                                                    <div class="d-flex flex-column gap-3">
                                                        @foreach ($perms as $perm)
                                                            <div class="form-check">
                                                               ...
                                                               {{ in_array(data_get($perm, 'id'), old('permission_ids') ?? []) || in_array(data_get($perm, 'name'), old('permission_ids') ?? []) ? 'checked' : '' }}>
                        ...
                        <div class="card-footer bg-white py-4 d-flex justify-content-end border-top">
                            ...
                            <button type="submit" class="btn btn-primary px-4 d-flex align-items-center bg-orange border-orange">
                                <i class="ti ti-circle-plus me-2"></i> Save Role
                            </button>
                        </div>
                    </form>
                </div>
